...
/**
 * Initialize directory token loading for all chains
 * This should be called once on app startup
 */
export const initializeDirectoryTokens = (): void => {
	...
	for (const chain of chains) {
		getDirectoryTokens$(chain.id).subscribe({
			next: ({ status, tokens, error }) => {
				if (error && tokens.length === 0) {
					// Error with no cached data
					logger.error(`Failed to load directory tokens for ${chain.id}`, {
						error,
					});
					statusByChain$.next({
						...statusByChain$.value,
				...
				for (const tokenId of Object.keys(newTokens)) {
					if (newTokens[tokenId]?.chainId === chain.id) {
			...
			error: (err) => {
				logger.error(`Directory tokens subscription error for ${chain.id}`, {
					err,
				});
				statusByChain$.next({
					...statusByChain$.value,
